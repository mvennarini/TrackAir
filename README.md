...
- **Two links, same encryption**: Wi-Fi (Bonjour + UDP) or **Bluetooth LE**,
  chosen automatically or by hand. Bluetooth works without any network, on a
  train or in a meeting room; Wi-Fi has the lowest latency at home. The Wi-Fi
  link keeps the phone's radio awake so the pointer never stalls.
- Mac app in the menu bar: paired devices, launch at login, Accessibility
  helper, Bluetooth status.
...
7. Optional: in Settings → **Connection** choose the link. *Automatic* uses
   Wi-Fi and falls back to Bluetooth when the Mac does not answer; *Bluetooth*
   needs no network at all (turn Bluetooth on in Control Center; iOS asks for
   the Bluetooth permission the first time). The same pairing is valid on both
   links.
...
| | Wi-Fi | Bluetooth LE |
|---|---|---|
| latency | 5–20 ms | 15–30 ms, very steady |
| update rate | native touch rate, up to 120 per second | about 60 per second (connection interval) |
| needs a router | yes, same network | no |
| weak spots | power-save gaps, busy networks, VM bridges | shares 2.4 GHz with Wi-Fi |

Both carry the same authenticated, encrypted frames. On the Mac, incoming
movement is applied at 240 Hz through a small filter that keeps sub-pixel
precision and bridges short gaps, so both links feel continuous.
...
Mac/                         menu bar app: Server (UDP), BLEServer (Bluetooth peripheral),
                             WebServer (HTTP + WebSocket), MouseController (CGEvent),
                             PairingController, SphereMetalView
Mac/trackpad.html            the web client
Shared/BLE.swift             Bluetooth service UUIDs and frame chunking
iOS/                         UIKit lifecycle + SwiftUI: Client (Wi-Fi + Bluetooth links),
                             BLEClient, TrackpadView (gesture engine), KeyboardBridge,
